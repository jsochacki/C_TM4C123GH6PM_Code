...
*/
	...
	unsigned long QA_power_word, QA_enableValue, QB_power_word, QB_enableValue, MuteUntilLDValue, OutDoublerEnValue;
	...
	if(QA_enable){ QA_enableValue = BIT_4; }
	else if(!QA_enable){ QA_enableValue = 0; }

	if(MuteUntil_LockDetect){ MuteUntilLDValue = BIT_5; }
	else if(!MuteUntil_LockDetect){ MuteUntilLDValue = 0; }

	if(QB_enable){ QB_enableValue = BIT_4; }
	else if(!QB_enable){ QB_enableValue = 0; }

	if(OutDoublerEnable){ OutDoublerEnValue = BIT_7; }
	else if(!OutDoublerEnable){ OutDoublerEnValue = 0; }

	unsigned long Reg34hValue, Reg34hWord, Reg36hValue, Reg36hWord, Reg3BhValue, Reg3BhWord;

	Reg34hValue = MuteUntilLDValue | QA_enableValue | BIT_3;

	Reg34hWord = Create24BitWord(Reg34hValue, REG_34h);
	sendWord_24Bit(Reg34hWord);

	Reg36hValue = QB_enableValue | BIT_3;

	Reg36hWord = Create24BitWord(Reg36hValue, REG_36h);
	sendWord_24Bit(Reg36hWord);

	Reg3BhValue = OutDoublerEnValue | OutDivider;

	Reg3BhWord = Create24BitWord(Reg3BhValue, REG_3Bh);
	sendWord_24Bit(Reg3BhWord);
